...
if (nodeVersion.major < 13) {
    logger(`Phiên bản Nodejs hiện tại của bạn: ${process.version} không được hỗ trợ, vui lòng cập nhật phiên bản Nodejs 13+ để có thể sử dụng!`, "error");
...
const dashboard = http.createServer(function (_req, res) {
    ...
    res.write("Simple Manga Network Messenger Bot, Code by Team MiraiV2 and Remaster by Dragonx943!");
    res.end();
});

dashboard.listen(process.env.port || 0);

logger("Đang tạo trang Web dành cho các trang treo Uptime!", "[ Đang khởi động ]");
...
function startBot(message) {
    (message) ? logger(message, "[ Đang khởi tạo ]") : "";
    ...
    child.on("error", function (error) {
        logger("An error occurred: " + JSON.stringify(error), "[ Đang khởi động lại với lỗi! ]");
    });
};
...
}).catch(err => logger("Hiện tại NettruyenBot không thể cập nhật!.", "[ Lỗi! ]"));
