...
if (nodeVersion.major < 13) {
    logger(`Phiên bản Nodejs ${process.version} không được hỗ trợ, vui lòng cập nhật phiên bản Nodejs v13+ để sử dụng!`, "error");
    return process.exit(0);
};

const dashboard = http.createServer(function (_req, res) {
    res.writeHead(200, "OK", { "Content-Type": "text/plain" });
    res.write("OK, Code bởi Team MiraiV2 và làm lại bởi dragonx943!");
    res.end();
});

dashboard.listen(process.env.port || 0);

logger("Tạo link dành cho các trang Uptime!", "[ OK ]");
...
function startBot(message) {
    ...
    child.on("error", function (error) {
        logger("Đã có lỗi xảy ra: " + JSON.stringify(error), "[ Lỗi! ]");
    });
};
